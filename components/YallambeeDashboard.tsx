...
import { managementPlanInputs, persistedDashboardPlan, persistedSchedule, persistedSummary, type ManagementPlanInput } from '@/lib/ui/persisted-optimizer-output';
...
function Timeline({ plan, disrupted }: { plan: OptimiserCandidate; disrupted: boolean }) {
  ...
  const clock = (hour: number) => `${String((6 + hour) % 24).padStart(2, '0')}:00`;
  return (
    <div className="yc-timeline">
      <div className="yc-timeline-toolbar"><span>Display window: hours {startHour}–{endHour}</span><label htmlFor="timeline-range">Start time<select id="timeline-range" value={startHour} onChange={(event) => setStartHour(Number(event.target.value))}>{[0, 6, 12, 18, 24, 30].map((hour) => <option key={hour} value={hour}>Hour {hour} · {clock(hour)}</option>)}</select></label></div>
      <div className="yc-timeline-scroll"><div className="yc-timeline-content"><div className="yc-timeline-hours">{Array.from({ length: 10 }, (_, index) => startHour + index * 2).map((hour) => <span key={hour}>{clock(hour)}</span>)}</div>
      <div className="yc-timeline-stage">
        {disrupted && 17.1 >= startHour && 17.1 <= endHour && <div className="yc-rain-shade" style={{ left: `${((17.1 - startHour) / span) * 100}%` }}><b>FRONT 23:18 · 22 MM</b></div>}
        {rows.map((row) => {
          const blocks = plan.blocks
            .filter((block) => block.m === row && block.s < endHour && block.e > startHour)
            .map((block) => ({ ...block, s: Math.max(startHour, block.s) - startHour, e: Math.min(endHour, block.e) - startHour }));
          return (
            <div className="yc-lane" key={row}>
              <span>{labels[row]}</span>
              <div className="yc-track">
                {blocks.map((block, index) => <i className={`yc-block ${cropClass[block.crop]}`} key={`${row}-${index}`} style={{ left: `${(block.s / span) * 100}%`, width: `${Math.max(3, ((block.e - block.s) / span) * 100)}%` }}>{block.name}</i>)}
                {!blocks.length && <i className="yc-block yc-idle" style={{ left: 0, width: '100%' }}>{disrupted ? 'No assignment in window' : 'No assignment'}</i>}
              </div>
            </div>
          );
        })}
